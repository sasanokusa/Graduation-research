...
- Minecraft log path
- `monitoring.ignored_failed_units`
- `monitoring.ignored_journal_patterns`
...
Minecraft の log が `/home/<user>/...` 配下にある場合は、監視実行ユーザーがその path を読める必要があります。
監視実行ユーザーを Minecraft 管理ユーザー本人に合わせるか、親ディレクトリを traverse できるように ACL / group 権限を付与してください。
権限不足の path は最新版コードではスキップされますが、ログ抜粋は取れません。
...
このとき、対象 service の restart だけを許可する限定的な `sudoers` 設定を追加してください。
例として `sasa` ユーザーが `apache2` だけ再起動できるようにする場合:

```bash
sudo visudo -f /etc/sudoers.d/infra-production-poc
```

```sudoers
sasa ALL=(root) NOPASSWD: /usr/bin/systemctl restart apache2
```
...
monitoring:
  ...
  ignored_journal_patterns:
    - connect_to localhost port 25565: failed
    - /var/log/playit/playit.log
```

`logrotate.service` が `playit.log` を参照して失敗している場合は、まず不要な `logrotate` 設定ファイルを確認してください。
それでもすぐに監視ノイズだけを抑えたい場合、この ignorelist を暫定的に使えます。
...
2. 必要なら `actions.restart_command_prefix: ["sudo", "-n"]` を設定する
3. `/etc/sudoers.d/infra-production-poc` に対象 service だけの `NOPASSWD` を入れる
4. `visudo -c` で構文確認する
5. `actions.mode` を `execute` に変更する
6. `monitor-once` を再実行する
7. 想定した service 名だけが restart 対象になることを確認する
