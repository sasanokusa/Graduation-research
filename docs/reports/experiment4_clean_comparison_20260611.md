...
### 4. self-critique の強さの解釈: 効いているのは役割分散ではなく反復観測

self-critique (23/24) は role-split (24/24) に肉薄したが、両者の実装差を正確に押さえる必要がある。

- self-critique は「全役割の単一エージェント化」ではなく、**planner と reviewer を同一モデル・同一エージェントが兼任する**構成である (`agents/self_critic.py`)。批判の判定ロジック (`parse_reviewer_text`、retry/stop、scope 推奨) は reviewer と共通で、システムプロンプトのみ「別人格の reviewer を立てず自己批判せよ」と指示している。judge 層と triage の LLM 化は持たない。
- したがって self-critique と one-shot の差 (23/24 vs 1/24) が示すのは**「再観測して計画し直すループの存在」そのものの価値**であり、self-critique と role-split の差 (`u` の 2/3 vs 3/3 のみ) が示すのが**批判役を外部化・別モデル化することの追加価値**である。本ベンチでは前者が支配的で、後者は誤仮説固着が起きた局面に限定して現れる。
- この結果は MAST (Cemri et al. 2025) の「multi-agent 構成の優位は single-agent 比で自明でない」という指摘と整合する。卒論では「協調の本質は役割分散より反復観測にあり、役割分散は固着破りの保険として効く」と整理できる。

### 5. self-critique 唯一の失敗 (`u_3`): 自己批判は自分の仮説を覆せないことがある

`u_3` は 2 ターンとも edit を出さない empty_plan で失敗した。`MySQL connection refused to 127.0.0.1` の evidence から「DB host 誤設定」仮説に到達した後、turn 2 でも同じ仮説・同じ観測範囲に留まり、安全側停止した。成功した `u_1`/`u_2` は 3 ターン使い「DB host 修正 → 露出した SQL typo を再観測して修正」と段階を踏んでいる。

role-split の `u` 3/3 では、Claude reviewer が「DB 接続は回復済み、残るは query bug」と修正 scope を明示的に押し直しており、**外部化された批判者が初期仮説の固着を破る**動きが観測できる。`u` は本実験で唯一、役割分散の追加価値が成功率に現れたシナリオであり、turn-by-turn ケーススタディの第一候補である (`m`/`o`/`r` = 観測解放の例、`v`/`w` = one-shot 脆性の例と並ぶ)。

### 6. 公平性の傍証

one-shot の 1/24 という低さは、隠れた正解情報が一切渡っていないことの傍証でもある。仮に修正内容に近い情報が prompt に混入していれば、回復ターンのない one-shot こそ高成功率になるはずだが、実際は観測粒度の罠 1 つで全滅している。blind prompt / forced scenario / restore 禁止の統制が機能している。
...
- 中間発表の畳み込みは **one-shot / self-critique / role-split** の 3 条件を推奨する。「単発 → 反復 (単一エージェント) → 反復 + 役割分散」と複雑さが 1 軸で増え、(a) 反復観測の価値 (1/24 → 23/24)、(b) 役割分散の追加価値は固着破りに限定 (`u`)、という本実験の 2 つの主張をそのまま提示できる。reviewer-only / reviewer+judge は本文の表にのみ残す。
