...
## 2026-05-08 時点の研究ステータス

機能開発としては、卒論に必要な核はおおむね揃っている。現在の主眼は、新しい修復機能を増やすことではなく、比較実験、仮説遷移メトリクス、図表、ケーススタディ、卒論本文への変換である。

現在地の詳細は [docs/current_status_20260508.md](docs/current_status_20260508.md) にまとめている。古い調査レポートやロードマップは履歴資料として残しているため、最新の進捗判断ではこの現在地メモを優先する。

現時点での主な実績:

- A-X の 24 シナリオを定義済み
- single-agent one-shot、single-agent iterative self-critique、multi-agent orchestration を実行可能
- `results/` に 200 件の result JSON、`observations/` に 9 本の summary.csv を保存済み
- `hypothesis_metrics` 入り result JSON は 13 件
- `./check.sh` は 99 passed, 6 deselected
- 予備実験では、hard scenario で single-agent one-shot より multi-agent が有利な傾向を確認済み

中間発表では、安全制約付き LLM 応急復旧基盤と multi-agent 再観測・再計画の予備効果までを出し、self-critique との詳細比較、仮説遷移分析、反復実験、卒論用図表化は今後の作業として残す。
...
- Triage agent:
  open-world fault-domain hypotheses と candidate scope を作る。前ターンの reviewer guidance がある場合は、残存ドメイン、推奨 scope、推奨追加観測を次ターンの planner input に反映する
...
卒論に向けた優先度は、機能追加よりも実験・図表・本文化の方が高い。拡張を行う場合は、比較実験の安定性や説明力に直結するものに絞る。

- `results/` の JSON と `observations/*/summary.csv` から、条件別の比較表を安定して生成する
- `hypothesis_metrics` を one-shot / self-critique / multi-agent の各条件で揃える
- hard scenario の turn-by-turn case study を図表化しやすい形式で出力する
- planner の scope 遵守や empty plan の原因分類を、実験集計に反映する
- 余力があれば dual-planner / planner diversity を追加比較として扱う
